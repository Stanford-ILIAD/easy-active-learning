...
def volume_objective_psi(psi_set, w_samples, delta):
    dR = w_samples.dot(psi_set.T)
	p1 = 1/(1+np.exp(delta - dR))
	p2 = 1/(1+np.exp(delta + dR))
	p_Upsilon = (np.exp(2*delta) - 1) * p1 * p2
	return p1.sum(axis=0)**2 + p2.sum(axis=0)**2 + p_Upsilon.sum(axis=0)**2
	
def information_objective_psi(psi_set, w_samples, delta):
	M = w_samples.shape[0]
	dR = w_samples.dot(psi_set.T)
	p1 = 1/(1+np.exp(delta - dR))
	p2 = 1/(1+np.exp(delta + dR))
...
def volume_objective(inputs_set, *args):
    ...
    return volume_objective_psi(psi_set, w_samples)
	
def information_objective(inputs_set, *args):
    simulation_object = args[0]
    w_samples = args[1]
    psi_set = generate_psi(simulation_object, inputs_set)
    return information_objective_psi(psi_set, w_samples)

def optimize(simulation_object, w_samples, func):
    z = simulation_object.feed_size
...
def volume(simulation_object, w_samples):
    return optimize(simulation_object, w_samples, volume_objective)
	
def information(simulation_object, w_samples):
    return optimize(simulation_object, w_samples, information_objective)
