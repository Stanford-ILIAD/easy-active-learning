...
class RobotEnv(GoalEnv):
    ...
    def step(self, action):
        old_achieved_goal = self._get_obs()
        ...
        return self.get_features(obs['achieved_goal'], old_achieved_goal['achieved_goal'], self.goal, info), reward, done, info


    def get_features(self, achieved_goal, old_achieved_goal, goal, info):
        ...
        table_dist = 25 * -np.exp(-np.sqrt((x[2] - table_pos[2]) ** 2))
        obstacle_dist = 40 * (1 - np.exp(
            -np.sqrt((x[0] - obstacle_pos[0]) ** 2 + (x[1] - obstacle_pos[1]) ** 2 + (x[2] - obstacle_pos[2]) ** 2)))
        speed = 300 * np.linalg.norm(x[:3] - old_achieved_goal[:3])

        return np.array([goal_dist, table_dist, obstacle_dist, speed])
